"""macOS Eye Contact MVP V3.

V3 is a debugging/tuning version of V2. It makes detection status visible all
the time and lets you toggle correction on/off live so you can verify whether
tracking and correction are actually happening.

Run:
    python mac_mvp/eye_contact_mvp_v3.py --debug

Keys:
    q or Esc  quit
    d         toggle landmarks/boxes
...
def _draw_hud(frame, lines, face_detected: bool) -> None:
    ...
    cv2.rectangle(frame, (0, 0), (frame.shape[1], 72), bg_color, -1)
    for line in lines:
        cv2.putText(frame, line, (x, y), cv2.FONT_HERSHEY_SIMPLEX, 0.68, (255, 255, 255), 2, cv2.LINE_AA)
        y += 28
    ...
    try:
        while True:
            ...
            _draw_hud(
                frame,
                [
                    f"v3 face={'detected' if face_detected else 'missing'} {iris_status} fps={current_fps:.1f}",
                    f"correction={'on' if correction_enabled else 'off'} strength={strength:.2f} smooth={args.smoothing:.2f} debug={'on' if debug else 'off'}",
                ],
                face_detected,
            )
            ...
            cv2.imshow("Mac Eye Contact MVP V3", frame)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Visible-status macOS eye-contact MVP V3")
    ...
    parser.add_argument("--deadzone", type=float, default=1.8)
